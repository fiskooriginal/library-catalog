...
async def main():
    ...
    if not api_key:
        print("Ошибка: Не установлена переменная окружения JSONBIN_API_KEY")
        print("Получите API ключ на https://jsonbin.io/ и установите его:")
        print("  export JSONBIN_API_KEY='your_api_key_here'")
        return

    bin_id = os.getenv("JSONBIN_BIN_ID")

    repo = JSONBinRepository(api_key=api_key, bin_id=bin_id)

    print("=== JSONBin.io Repository Demo ===")
    print(f"API Key: {api_key[:10]}...")
    print(f"Bin ID: {bin_id if bin_id else 'Будет создан новый'}\n")
    ...
    if not bin_id:
        print(f"\n🎉 Создан новый Bin с ID: {repo.get_bin_id()}")
        print("Сохраните его для дальнейшего использования:")
        print(f"  export JSONBIN_BIN_ID='{repo.get_bin_id()}'")
        print(f"Просмотреть данные: https://jsonbin.io/app/bins/{repo.get_bin_id()}\n")
