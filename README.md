...
- Структурированное логирование с поддержкой JSON и цветного вывода
- Middleware для логирования запросов и мониторинга производительности
...
│   ├── cache/           # Реализация кеширования (Redis)
│   ├── http/            # HTTP клиенты
│   ├── middleware/      # Middleware (логирование, тайминг)
...
- **PostgreSQL** 17+ (для работы с базой данных)
- **Redis** 7+ (для кеширования, опционально)
...
### 3. Настройка базы данных и Redis

Используйте значения из `.env` файла для создания базы данных PostgreSQL и настройки Redis (если используется).

**Примечание**: Redis опционален, но рекомендуется для повышения производительности. Приложение будет работать без Redis, но без кеширования.
...
- **postgres** — база данных PostgreSQL 17
- **redis** — кеш Redis 7
- **postgres_migration** — сервис для применения миграций
- **postgres_backup** — автоматическое резервное копирование БД (опционально)
...
- **DB_HOST** — хост базы данных (в Docker используйте `postgres`)
...
- **REDIS_HOST** — хост Redis (в Docker используйте `redis`)
- **REDIS_PORT** — порт Redis (по умолчанию `6379`)
- **REDIS_DB** — номер базы данных Redis (по умолчанию `0`)
- **REDIS_PASSWORD** — пароль Redis (опционально)
- **REDIS_URL** — полный URL подключения к Redis (опционально, имеет приоритет над отдельными параметрами)

**Резервное копирование:**

- **POSTGRES_BACKUP_SCHEDULE** — расписание бэкапов в формате cron (по умолчанию `@daily`)
- **POSTGRES_BACKUP_DIR** — директория для хранения бэкапов (по умолчанию `./.backups/postgres`)
...
- Убедитесь, что база данных готова: `docker-compose logs postgres`
- Проверьте переменные окружения в `.env` файле

**Проблема**: Ошибки подключения к базе данных

- Убедитесь, что `DB_HOST=postgres` в переменных окружения
- Проверьте, что сервис `postgres` запущен: `docker-compose ps`
- Проверьте healthcheck базы данных: `docker-compose ps postgres`

**Проблема**: Ошибки подключения к Redis

- Убедитесь, что `REDIS_HOST=redis` в переменных окружения
- Проверьте, что сервис `redis` запущен: `docker-compose ps`
- Проверьте healthcheck Redis: `docker-compose ps redis`
- Приложение будет работать без Redis, но без кеширования
...
- `LOG_JSON` — использовать JSON формат для логов (по умолчанию: `false`)
...
- **Application** — use cases и бизнес-сценарии, сервисы кеширования
- **Infrastructure** — реализация технических деталей (БД, HTTP клиенты, Redis, middleware)
- **Presentation** — API эндпоинты и схемы валидации

### Кеширование

Проект использует Redis для кеширования данных:

- **Кеширование книг** — отдельные книги кешируются по UUID
- **Кеширование списков** — результаты списков книг кешируются с учетом фильтров и пагинации
- **Автоматическая инвалидация** — кеш автоматически инвалидируется при изменении данных
- **Graceful degradation** — при недоступности Redis приложение продолжает работать без кеширования

### Система логирования

Проект использует структурированное логирование:

- **Цветной вывод** — для разработки (по умолчанию)
- **JSON формат** — для production (устанавливается через `LOG_JSON=true`)
- **Middleware логирование** — автоматическое логирование всех HTTP запросов
- **Тайминг** — отслеживание времени выполнения запросов через `X-Process-Time` заголовок
- **Медленные запросы** — автоматическое предупреждение для запросов > 1 секунды

### Middleware

Приложение включает следующие middleware:

- **LoggingMiddleware** — логирование всех HTTP запросов и ответов
- **TimingMiddleware** — измерение времени выполнения запросов и добавление заголовка `X-Process-Time`
